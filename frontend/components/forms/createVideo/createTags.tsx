...
export const CreateTags = ({tags, injectTag, deleteTag}: { tags: {tag: string, id: number}[], injectTag: (tag: {tag: string, id: number}) => void, deleteTag: (id: number) => void}) => {
  ...
  const handleKeyDown  = (event: any) => {
    if(event.key=='Enter') {
      if(tags.length>5 || tags.find( t => t.tag===value) || value==='' || value.length>10 || value.length<4) return;
  ...
  return <div>
    ...
    <div className={"flex  border border-black p-5 m-2 rounded-md space-x-2 space-y-1 flex-wrap"}>
      { tags && tags.map( (t) => <div className={"flex border px-5 py-2 rounded-full border"} key={t.id}>#{t.tag}<button className={"border  p-1 ml-1 rounded-full"} onClick={() => deleteTag(t.id)}><svg width="10" height="10" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg">
