...
export const SubscribeChannel = ({video, user, isSubscribe, setIsSubscribe}: { video: IVideo, user?: IUser, isSubscribe: boolean ,setIsSubscribe: () => void}): JSX.Element => {
  const followChannel = () => {
  ...
  return <div className="z-10 ml-2 absolute flex justify-between my-8 w-80" >
    {
      isSubscribe  ?<>
          ...
          {
            video.authorVideo.id === user?.id ?       <ButtonDeleteVideo  deleteVideo={() => deleteVideoHandler(video.id)}/>  :  <Button onClick={() => setIsSubscribe} className={"bg-red-500 h-12 border-red right-0"}>Отписаться</Button>
          }
      </>
      ...
      :
        ...
        {
          video.authorVideo.id === user?.id ?       <ButtonDeleteVideo deleteVideo={() => deleteVideoHandler(video.id)} />  :  <Button onClick={() => setIsSubscribe} className={"bg-red-500 h-12 border-red right-0"}>Подписаться</Button>
        }
        </>
    }
  </div>
