...
import {
  ...
  useLikeVideoMutation, useUnfollowChannelMutation, useWatchVideoMutation
...
export const Video = ({ user, video, className, ...props}: VideoProps): JSX.Element => {
  ...
  const audioEl = new Audio('http://localhost:8000/user/' + video.music.music_url);
  useEffect(() => {
    const audioEl = new Audio('http://localhost:8000/user/' + video.music.music_url);
  ...
  const handleEndVideo = async () => {
    console.log(1122211);
    if(user) { console.log(1122211);await watchVideo({videoId: video.id});}
  ...
  const onVideoClick = () => {
    ...
    if (isPlaying) {
      ...
      if(!isWatching) { console.log(1); handleEndVideo();}
      setIsWatching(true)
